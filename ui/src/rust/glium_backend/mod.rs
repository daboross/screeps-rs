//! An inlined version of conrod::backend::glium for experimentation.
#![cfg_attr(rustfmt, rustfmt_skip)]

use glium;

use std;
use conrod::{color, image, render, text, Rect, Scalar};
...
impl Renderer {
    ...
    pub fn commands(&self) -> Commands {
        let Renderer { ref commands, .. } = *self;
        Commands {
              ...
              for<'a> glium::uniforms::Sampler<'a, T>: glium::uniforms::AsUniformValue,
    {
        ...
        let vbuffer = glium::VertexBuffer::new(facade, &self.vertices)?;
            ...
            match command {
                ...
                Command::Draw(draw) => match draw {
                    ...
                    Draw::Plain(slice) => if slice.len() >= NUM_VERTICES_IN_TRIANGLE {
                        let vertex_buffer = vbuffer.slice(slice).unwrap();
