//! This contains text widgets which are essentially copies of the widgets from Conrod, but with added support for
//! hiding characters being typed (for passwords, etc.).
//! Text layout logic.

pub mod text_box {
    use conrod::{Borderable, Color, Colorable, FontSize, Positionable, Sizeable, Widget};
    use conrod::{event, input, text, widget};
    use conrod::position::{Range, Rect, Scalar};
    ...
    impl<'a> Widget for TextBox<'a> {
        ...
        fn update(self, args: widget::UpdateArgs<Self>) -> Self::Event {
            ...
            if let Some(new_string) = TextEdit::new(text)
                .and_then(font_id, TextEdit::font_id)
                .wh(text_rect.dim())
                .xy(text_rect.xy())
                .font_size(font_size)
                .color(text_color)
...
pub mod text_edit {
    use std;
    use conrod::{Color, Colorable, FontSize, Positionable, Sizeable, Ui, Widget};
    use conrod::{event, input, text, utils, widget};
    ...
    impl<'a> Widget for TextEdit<'a> {
        ...
        fn update(self, args: widget::UpdateArgs<Self>) -> Self::Event {
            ...
            let mut replacement_string = style
                .char_replace
                .and_then(|opt| opt)
             ...
             -> Option<(String, Cursor, std::vec::Vec<text::line::Info>)> {
                ...
                let (new_text, new_cursor_char_idx): (String, usize) = {
                    let (cursor_start, cursor_end) = match cursor {
                        ...
                        Cursor::Selection { start, end } => (std::cmp::min(start, end), std::cmp::max(start, end)),
                    };
                ...
                let new_line_infos: Vec<_> = { line_infos(&new_text, font, font_size, line_wrap, rect.w()).collect() };
                ...
                match widget_event {
                    event::Widget::Press(press) => match press.button {
                        ...
                        event::Button::Mouse(input::MouseButton::Left, rel_xy) => {
                            ...
                            let font = ui.fonts.get(font_id).unwrap();
                            let closest = closest_cursor_index_and_xy(
                                abs_xy,
                                ...
                                if let (Some(start_idx), Some(end_idx)) = (start_idx, end_idx) {
                                    ...
                                    *text.to_mut() = text.chars()
                                    ...
                                    state.update(|state| {
                                        let font = ui.fonts.get(font_id).unwrap();
                                        let w = rect.w();
                                        state.line_infos = line_infos(&text, font, font_size, line_wrap, w).collect();
                                    });
                                }
                            }
                                        ...
                                        Cursor::Selection { start, end } => {
                                            ...
                                            let new_cursor_idx = {
                                                let cursor_idx = match key {
                                                    input::Key::Left | input::Key::Up => std::cmp::min(start, end),
                                                    input::Key::Right | input::Key::Down => std::cmp::max(start, end),
                                                    _ => unreachable!(),
                                                };
